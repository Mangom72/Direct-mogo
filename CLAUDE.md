...
**수능 시행일의 관례는 이렇다** (저장소 자료 21회를 전수 조사한 결과).

- **요일 — 예외 없이 목요일.** 2007학년도부터 그렇고, 문제지 배송 차량이 주말
  고속도로 혼잡을 피하도록 그때 바꾼 것이다. 21회 전부 목요일이었다
- **날짜 — 11월 13~19일 사이의 목요일.** 7일 창이라 목요일이 딱 하나뿐이다.
  2016년 시행분부터 **12회 연속 예외 없음**. 지진·코로나로 연기된 두 해도
  '연기 전 예정일'은 창 안이었다(2017-11-16, 2020-11-19)
- 어긋나는 옛 해는 2009·2011·2012·2013·2015뿐이고 **전부 정확히 일주일 빨랐다**
- "11월 셋째 목요일"이라는 통설은 20회 중 9회만 맞는다. 쓰지 말 것

**그래도 계산하지 않고 해마다 손으로 고친다.** 관례는 평가원이 해마다 공고하는
것이지 규칙이 아니고, 실제로 2015년 이전에는 자주 어긋났다. `index.html`의
`const SUNEUNG` 한 줄이 전부이고, 날이 지나면 화면에서 조용히 사라진다 — 다음
시험일이 아직 공고되지 않았을 수 있는데 그 사이에 'D+12'를 띄우면 고쳐지지 않은
화면이 스스로 틀린 소리를 하게 된다. 지금 값은 `2026-11-19`(2027학년도).

잊지 않도록 매일 도는 갱신이 `tests/test_dday.py --date`를 부른다 — 날이 지났거나,
목요일이 아니거나, 한국어 위키백과의 학년도별 인포박스와 어긋나면 실패한다.
13~19일 창을 벗어난 것은 말만 하고 넘어간다(평가원이 관례를 바꿨을 수 있다). **위키백과 값을 화면에
쓰지는 않는다.** 누구나 고칠 수 있는 문서라 틀린 날짜가 조용히 나가는 것보다
아무것도 안 뜨는 편이 낫고, 공식 쪽에는 기계가 읽을 출처가 아예 없다(평가원 RSS
없음, 공공데이터포털에도 없음).
