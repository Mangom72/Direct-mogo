...
**4층 — 백업 파일.** 푸터의 **백업**에서 파일 하나로 내보내고 가져옵니다.
...
- 이름은 `기출직행-백업-20260824.json`. 표시가 5,059개라도 300KB 남짓입니다.
...
기기끼리 **저절로** 맞춰지지는 않습니다. 그러려면 계정과 서버가 필요하고, 그것은
이 저장소가 하지 않기로 한 것입니다. 사람이 파일을 옮기는 것이 지금의 답입니다.
