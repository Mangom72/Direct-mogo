...
시행일은 상수 한 줄이라 해마다 손으로 고쳐야 하는데, 그날이 지나는 시점은 대개
아무도 코드를 밀지 않는 때라 `tests.yml`로는 영영 안 잡힙니다. 자료 커밋 뒤에
두어서, 날짜가 지났어도 그날의 자료는 그대로 나가고 워크플로만 빨갛게 뜹니다.

이때 한국어 위키백과의 학년도별 인포박스와도 대조합니다. **그 값을 화면에 쓰지는
않습니다** — 누구나 고칠 수 있는 문서라, 틀린 날짜가 조용히 나가는 것보다 아무것도
안 뜨는 편이 낫습니다. 어긋날 때 사람을 부르는 데에만 씁니다. 닿지 못하면 그냥
넘어갑니다. 인포박스를 읽는 규칙 자체는 망 없이 `test_dday` 안에서 견줍니다.
