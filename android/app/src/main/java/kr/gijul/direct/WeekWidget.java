...
public class WeekWidget extends WidgetBase {
    ...
    private Bitmap bars(Context c, int[] n, int w, int h) {
        ...
        float gap = px(c, 5), cw = (w - gap * 6) / 7f;
        float lab = px(c, 11), foot = px(c, 10);
        ...
        for (int i = 0; i < 7; i++) {
            float x0 = i * (cw + gap);
            p.setTextSize(px(c, 8));
            p.setColor(i == 0 ? color(c, R.color.w_mark) : color(c, R.color.w_ink2));
            g.drawText(dow[i], x0 + cw / 2, lab - px(c, 3), p);

            float bh = Math.max(0, bot - top);
            p.setColor(color(c, R.color.w_rule));
            g.drawRoundRect(x0, top, x0 + cw, bot, px(c, 3), px(c, 3), p);
            if (n[i] > 0) {
                float fill = bh * n[i] / max;
                p.setColor(color(c, R.color.w_mark));
                g.drawRoundRect(x0, bot - fill, x0 + cw, bot, px(c, 3), px(c, 3), p);
            }
            ...
            if (n[i] > 0) {
                ...
                g.drawText(String.valueOf(n[i]), x0 + cw / 2, h - px(c, 1), p);
            }
        }
        return b;
    }
