...
2006년 시행분부터 **5천여 회차 · 77과목** — 국어·수학·영어·한국사, 사회·과학탐구에
...
- **오프라인 저장** — 회차의 문제·정답·해설을 한 번에 담아 두고 연결이 없어도 엽니다. **웹에서도 됩니다** — 앱은 폴더에, 브라우저는 보관함에 담기고, 담아둔 회차에는 목록에 표시가 붙습니다. 파일 자체가 필요하시면 '파일로 내려받기'가 따로 있습니다
...
- **홈 화면 위젯 여섯** — 달력 · 이번 주 · 잔디밭 · 최근 푼 것 · D-day와 진도 · 다음에 풀 것. 앱과 같은 글꼴로 그려지고 화면 크기에 맞게 자랍니다. 고르는 화면에 미리보기가 뜨고, 누르면 그 위젯이 말하던 자리로 갑니다 — 푼 것을 보여 주던 넷은 달력으로, '다음에 풀 것'은 줄마다 그 과목으로
- **푼 날 달력** — 진도 옆 달력 단추를 누르면 열립니다. 과목을 가리지 않고 날짜로 되짚습니다. 태블릿은 오른쪽에, 폰은 아래에 그날 푼 것이 펼쳐집니다
- **여러 기기 합치기**(앱) — 자동 백업이 쓰는 파일을 드라이브 안에 두고 다른 기기가 같은 파일을 가리키면, 열 때마다 서로 합쳐집니다. **로그인도 서버도 없습니다** — 파일 선택기에 나오는 곳이면 원드라이브든 삼성 클라우드든 똑같습니다
- **백업** — 계정도 서버도 없으므로 이 기기 밖에는 아무것도 없습니다. 앱에서는 **자리를 한 번 고르면 그 뒤로는 저절로** 씁니다 — 드라이브 같은 동기화 폴더로 두면 그것이 곧 클라우드 백업입니다. 손으로 파일을 내보내고 가져오는 길도 있고(웹에도 있습니다), 합치기가 기본이라 다른 기기 것을 가져와도 여기서 찍은 것은 남습니다
- **시험 시간 재기**(앱) — 뷰어의 타이머 아이콘으로 시작합니다. 과목마다 정해진 시간이 뜨고(영어는 듣기 없이 푸는 50분도), 뷰어·띄운 창·알림 세 곳에서 같은 시계가 흐릅니다. 끝내면 `92분 · 8분 남김`처럼 회차에 남습니다
- **다크 모드** — 자동 / 밝게 / 어둡게
...
받을 앱은 없지만 **공유 → 홈 화면에 추가**로 넣으시면 같은 화면이 앱처럼 열립니다.
그러면 담아둔 자료와 푼 기록이 **브라우저 정리에 지워지지 않습니다** — 사파리 탭에
그냥 두면 이레 동안 안 들어올 때 사라질 수 있습니다.

한 가지만 주의하십시오. **홈 화면 앱과 사파리는 기록을 따로 씁니다.** 같은 주소인데
아이콘으로 연 것과 사파리로 연 것이 서로를 못 봅니다. 사파리에서 표시해 두신 것이
있으면 **백업으로 내보낸 뒤 홈 화면에서 열어 가져오십시오.** 화면이 이 안내를 먼저
띄웁니다.

무엇이 되고 무엇이 안 되는지는 [docs/ios.md](docs/ios.md)에 하나씩 있습니다.
