...
쪽에서는 압축 덩어리만 보인다. 같은 자료를 평범한 JSON으로도 함께 배포해
서버 없이 조회할 수 있게 한다.
...
def kind_of(title):
    ...
    """
    # 예비시행은 이름이 두 가지로 온다 — '예비시행 …' 처럼 앞에 오기도 하고,
    # '2014 수능 예비시행(평가원)' 처럼 가운데 박히기도 한다. 뒤엣것은 '수능'을
    # 품고 있어서, 앞에서 걸러 내지 않으면 수능으로 잘못 세어진다.
    if "예비시행" in title or title.startswith("예비"):
        return "예비시행"
...
def paper(row, year, grade, sid):
    ...
    out = {
        ...
        # 붙인다 — 그러면 5,059회차가 모두 하나씩 구분된다. 자료는 덧붙이기만
...
| 갱신 주기 | 매일 15:23·23:23(KST) 자동. 목록 반영이 늦어도 다음 실행에서 보충합니다 |
